...
// Expanded quizzes reflecting mindful eating and emotional healing (tags removed, tone softened)
const quizzes = [
...
// Dialogues by level to rotate periodically
const dialogues = {
    bad: [
        "마스터... 기운이 하나도 없어...",
        "나도 아프고 너도 아프면 어떡해 😢",
...
const levels = [
    { maxScore: 19, name: '겁나 아픈 푸디몬', img: 'assets/level1.jpg' },
    { maxScore: 39, name: '아픈 푸디몬', img: 'assets/level2.jpg' },
    { maxScore: 59, name: '기본 푸디몬', img: 'assets/level3.jpg' },
    { maxScore: 79, name: '건강한 푸디몬', img: 'assets/level4.jpg' },
    { maxScore: 94, name: '생기발랄 푸디몬', img: 'assets/level5.jpg' },
    { maxScore: 100, name: '천상계 푸디몬', img: 'assets/level6.jpg' }
];
...
function init() {
    ...
    storySection.addEventListener('click', handleStoryAdvance);
...
function handleStoryAdvance(e) {
    ...
    if (storyStep === 0) {
        ...
        storyActions.innerHTML = `<button class="primary-btn" onclick="acceptAdoption(event)">입양하기! 🥰</button>`;
        storyStep++;
    }
}

function acceptAdoption(e) {
    e.stopPropagation();
    storyText.innerHTML = `고마워! ${userState.masterName} 마스터!<br>그럼 마스터에 대해 알려줘.<br>평소 수면 시간은 어느 정도야?`;
    storyActions.innerHTML = `
        <select id="onboarding-sleep" style="margin-bottom: 12px;">
        ...
        <button class="primary-btn" onclick="nextStorySleep(event)">다음</button>
    `;
    storyStep++;
}

function nextStorySleep(e) {
    e.stopPropagation();
    userState.targetSleep = parseFloat(document.getElementById('onboarding-sleep').value);
    storyText.innerHTML = `하루 목표 식사 횟수는 몇 끼야?`;
    storyActions.innerHTML = `
        <select id="onboarding-meals" style="margin-bottom: 12px;">
            <option value="1">1끼</option>
            <option value="2">2끼</option>
            <option value="3" selected>3끼</option>
            <option value="4">4끼</option>
        </select>
        <button class="primary-btn" onclick="nextStoryMeals(event)">다음</button>
    `;
    storyStep++;
}

function nextStoryMeals(e) {
    e.stopPropagation();
    userState.targetMeals = parseFloat(document.getElementById('onboarding-meals').value);
    storyText.innerHTML = "마지막으로, 일반적인 1인분 기준으로 평소에 얼만큼 먹어?";
    storyActions.innerHTML = `
        <select id="onboarding-meal-size" style="margin-bottom: 12px;">
            <option value="0.33">1/3 이하</option>
            <option value="0.5">1/2</option>
            <option value="0.66">2/3</option>
            <option value="1.0" selected>1 (정량)</option>
            <option value="1.5">1.5</option>
            <option value="2.0">2 이상</option>
        </select>
        <button class="primary-btn" onclick="finishStory(event)">시작하기 ✨</button>
    `;
    storyStep++;
}
...
function finishStory(e) {
    e.stopPropagation();
    userState.usualMealSize = parseFloat(document.getElementById('onboarding-meal-size').value);
...
function handleMealLog(size) {
    ...
    if (size <= 0.33 || size >= 2.0) {
        userState.today.badMealCount++;
        showToast(`앗, 식사량이 조금 과하거나 부족했어요! 🤢`);
    } else {
